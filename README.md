...
- **自动化剪辑**：客户端实时合成视频，支持 1080x1920 (9:16) 竖屏画幅。
- **60帧高清合成**：导出流畅的 60 FPS WebM 视频。
...
2. **浏览器兼容性**: 视频合成功能使用了 `MediaRecorder` 和 `Canvas API`，在 Chrome/Edge/Firefox 桌面版上表现最佳。移动端浏览器可能对高分辨率 (1080x1920) 录制的内存限制较为严格。
3. **视频编解码**: 默认导出为 `WebM` (VP9) 格式，具有高画质和高压缩率。如果需要在 iOS 相册查看，可能需要转码为 MP4。
